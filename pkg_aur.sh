...
# Build plymouth with osmium-theme (Not a good way, i know)
build_plymouth () {
#	{ echo "Building plymouth with custom configs & osmium-theme"; echo; }
#	{ cd $DIR/aur_pkgs; git clone --depth 1 https://aur.archlinux.org/plymouth.git; echo; }
#	{ cd $DIR/aur_pkgs/plymouth; cp -r $DIR/plymouth/osmium $DIR/aur_pkgs/plymouth; cp -r $DIR/plymouth/text $DIR/aur_pkgs/plymouth; cp -r $DIR/plymouth/osmium-text.so $DIR/aur_pkgs/plymouth; }
#	sed -i '$d' PKGBUILD
#	cat >> PKGBUILD <<- EOL
#	  sed -i -e 's/Theme=.*/Theme=osmium/g' \$pkgdir/etc/plymouth/plymouthd.conf
#	  sed -i -e 's/ShowDelay=.*/ShowDelay=1/g' \$pkgdir/etc/plymouth/plymouthd.conf
#	  cp -r ../../osmium \$pkgdir/usr/share/plymouth/themes
#	  cp -r ../../text \$pkgdir/usr/share/plymouth/themes
#	  cp -r ../../osmium-text.so \$pkgdir/usr/lib/plymouth
#	}
#	EOL
...
# Setup repository
setup_repo () {
	repoargs=("-n -R osmium-pkgs.db.tar.gz *.pkg.tar.zst")
	{ echo "Setting up repository & updating database..."; echo; }
	{ cd $DIR/x86_64; rm -f osmium-pkgs.*; repo-add $repoargs; }
